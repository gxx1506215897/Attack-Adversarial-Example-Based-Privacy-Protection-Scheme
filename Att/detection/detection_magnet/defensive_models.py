...
from keras.layers import Input, Conv1D, MaxPooling1D, UpSampling1D, AveragePooling1D
    ...
    def __init__(self, image_shape,
        ...
        for layer in structure:
            if isinstance(layer, int):
                x = Conv1D(layer, 10, activation=activation, padding="same",
        ...
        decoded = Conv1D(c, 10, activation='sigmoid', padding='same',
                         activity_regularizer=regs.l2(reg_strength))(x)
        self.model = Model(input_img, decoded)

    def train(self, x_train, x_test, archive_name, clip_min, clip_max, num_epochs=50, batch_size=100,
              if_save=True):
