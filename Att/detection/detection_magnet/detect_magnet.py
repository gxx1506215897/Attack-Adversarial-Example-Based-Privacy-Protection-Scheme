...
from setup_paths import * # 包含 magnet_results_dir
...
def main(args):
    assert args.dataset in DATASETS, \
        "Dataset parameter must be either 'mnist', 'cifar', 'svhn', 'tiny', or 'location'"
    ATTACKS = ATTACK[DATASETS.index(args.dataset)]

    assert os.path.isfile('{}cnn_{}_defense.h5'.format(checkpoints_dir, args.dataset)), \
    ...
    if args.dataset == 'mnist':
        ...
        epochs=100
    elif args.dataset == 'attriguard':
        from baselineCNN.cnn.cnn_attriguard import AGCNN as myModel
        model_class = myModel(mode = 'load', mode_="defense", filename = ['cnn_{}_defense.h5'.format(args.dataset), 'cnn_{}_attack_robust.h5'.format(args.dataset)])
        model=model_class.attack_model
        ...
        from baselineCNN.cnn.cnn_location import Location_CNN as myModel
        model_class = myModel()
        model = model_class.attack_model
    ...
    val_size = 5000
    x_val = X_train[:val_size].reshape(-1, 10000, 1)
    y_val = Y_train[:val_size]
    X_train = X_train[val_size:].reshape(-1, 10000, 1)
    ...
    im_dim = [X_train.shape[1], X_train.shape[2]]
    ...
    preds_test = model.predict(X_test.reshape(-1,10000))
    ...
    #Make AEs ready
    classifier = Classifier(modelx, model_class.num_classes) # 这个在哪里定义？
    if type=='error':
    ...
    for attack in ATTACKS:
        ...
        X_test_adv = X_test_adv.reshape(-1, 10000, 1)
        ...
        pred_adv = model.predict(X_test_adv.reshape(-1, 10000))
        loss, acc_suc = model.evaluate(X_test_adv.reshape(-1,10000), Y_test, verbose=0)
